...
def test_eigen_parameters():
    ...
    eig_val1, eig_vec1 = test_gnm.eigen(copy=False, nzero=False)
    eig_val1[1] = 3
    eig_vec1[1, 1] = 3
    eig_val2, eig_vec2 = test_gnm.eigen(copy=False, nzero=False)
    assert np.array_equal(eig_val1, eig_val2)
    assert np.array_equal(eig_vec1, eig_vec2)

    test_gnm = prepare_gnm(file_path, cutoff)

    eig_val1, eig_vec1 = test_gnm.eigen(copy=True, nzero=False)
    eig_val1[1] = 3
    eig_vec1[1, 1] = 3
    eig_val2, eig_vec2 = test_gnm.eigen(copy=True, nzero=False)
    assert not np.array_equal(eig_val1, eig_val2)
    assert not np.array_equal(eig_vec1, eig_vec2)

    test_gnm = prepare_gnm(file_path, cutoff)

    eig_val1, eig_vec1, eig_nzero1 = test_gnm.eigen(copy=False, nzero=True)
    eig_val1[1] = 3
    eig_vec1[1, 1] = 3
    eig_val2, eig_vec2, eig_nzero2 = test_gnm.eigen(copy=False, nzero=True)
    assert np.array_equal(eig_val1, eig_val2)
    assert np.array_equal(eig_vec1, eig_vec2)
    ...
    eig_val1, eig_vec1, eig_nzero1 = test_gnm.eigen(copy=True, nzero=True)
    eig_val1[1] = 3
    eig_vec1[1, 1] = 3
    eig_val2, eig_vec2, eig_nzero2 = test_gnm.eigen(copy=True, nzero=True)
    assert not np.array_equal(eig_val1, eig_val2)
    assert not np.array_equal(eig_vec1, eig_vec2)
    assert eig_nzero1 == eig_nzero2
